...
def process_depth_image(env: BaseEnv, sensor_cfg: SceneEntityCfg, data_type: str, visualize=False, far_clip: float = 5.0, near_clip: float = 0.3) -> torch.Tensor:
    ...
    if visualize:
        depth_image_size = (output.shape[2], output.shape[3])
        output_clone = output.clone().reshape(
            env.num_envs, depth_image_size[0], depth_image_size[1])[0, :, :]
        window_name = "Depth Image"
        cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
        cv2.imshow("Depth Image", output_clone.cpu().numpy())
        cv2.waitKey(1)
...
def process_lidar(env: BaseEnv, sensor_cfg: SceneEntityCfg, offset: float = 0.5) -> torch.Tensor:
    ...
    # import pdb; pdb.set_trace()
    output = sensor.data.pos_w[:, 2].unsqueeze(
        1) - sensor.data.ray_hits_w[..., 2] - offset
    near_clip = 0.0
    far_clip = 5.0
    output[torch.isnan(output)] = far_clip
    output[torch.isinf(output)] = far_clip
...
def base_rpy(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    sinp = 2 * (qw * qy - qz * qx)
    pitch = torch.where(torch.abs(sinp) < 1, torch.asin(
        sinp), torch.sign(sinp) * (torch.tensor(torch.pi) / 2))
...
def height_map_lidar(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg, offset: float = 0.5) -> torch.Tensor:
    ...
    sensor_quat_default = torch.tensor(
        [-0.131, 0.0, -0.991, 0.0], device=robot_base_quat_w.device).unsqueeze(0).repeat(hit_vec_shape[0], 1)
    sensor_quat_w = math_utils.quat_mul(robot_base_quat_w, sensor_quat_default)
    quat_w_dup = (sensor_quat_w.unsqueeze(1).repeat(
        1, hit_vec_shape[1], 1)).view(-1, sensor_quat_w.shape[-1])
    hit_vec_lidar_frame = math_utils.quat_rotate_inverse(quat_w_dup, hit_vec)
    hit_vec_lidar_frame = hit_vec_lidar_frame.view(
        hit_vec_shape[0], hit_vec_shape[1], hit_vec_lidar_frame.shape[-1])

    num_envs = hit_vec_lidar_frame.shape[0]

    # Calculate the number of voxels in each dimension
    x_bins = torch.arange(
        range_x[0], range_x[1], voxel_size_xy, device=hit_vec_lidar_frame.device)
    y_bins = torch.arange(
        range_y[0], range_y[1], voxel_size_xy, device=hit_vec_lidar_frame.device)
    ...
    env_indices = torch.arange(num_envs, device=hit_vec_lidar_frame.device).unsqueeze(
        1).expand_as(valid_indices)
    flat_env_indices = env_indices[valid_indices]

    map_2_5D = torch.full((num_envs, len(x_bins), len(y_bins)), float(
        'inf'), device=hit_vec_lidar_frame.device)
    linear_indices = flat_env_indices * \
        len(x_bins) * len(y_bins) + x_indices * len(y_bins) + y_indices
    ...
    map_2_5D = map_2_5D.view(-1).scatter_reduce_(0,
                                                 linear_indices, z_filtered, reduce="amin") - offset
    map_2_5D = torch.where(map_2_5D < 0.05, torch.tensor(
        0.0, device=map_2_5D.device), map_2_5D)
    ...
    map_2_5D = torch.where(torch.isinf(map_2_5D), torch.tensor(
        0.0, device=map_2_5D.device), map_2_5D)
    ...
    max_across_frames = F.max_pool2d(
        map_2_5D, kernel_size=3, stride=1, padding=1).view(num_envs, -1)
